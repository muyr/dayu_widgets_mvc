...
class MOptionDelegate(QStyledItemDelegate):
    ...
    def createEditor(self, parent, option, index):
        ...
        attr = '{}_list'.format(model.header_list[real_index.column()].role('key'))
...
def set_header_list(self, header_list):
    ...
    if self.header_view:
        ...
        for index, column_obj in enumerate(header_list):
            self.header_view.setSectionHidden(index, column_obj.role('hide'))
            self.header_view.resizeSection(index, column_obj.role('width'))
            # if column_obj.role(Qt.InitialSortOrderRole) is not None:
            #     self.header_view.setSortIndicator(index, column_obj.role('order'))
            if column_obj.role('selectable'):
                delegate = MOptionDelegate(parent=self)
                delegate.set_exclusive(column_obj.role('exclusive'))
...
class MListView(QListView):
    ...
    def set_show_column(self, attr):
        for index, column_obj in enumerate(self.header_list):
