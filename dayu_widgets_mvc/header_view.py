...
from dayu_widgets import MMenu
...
class MHeaderView(QHeaderView):
    ...
    def _slot_context_menu(self, point):
        ...
        if logical_column >= 0 and model.header_list[logical_column].role('checkable', False):
    ...
    def _slot_set_select(self, column, state):
        ...
        attr = '{}_checked'.format(source_model.header_list[column].role('key'))
        for row in range(current_model.rowCount()):
            ...
            if state is None:
                ...
                utils.set_obj_value(data_obj, attr,
